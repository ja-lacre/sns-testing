...
export function ImportStudentsDialog({ open, onOpenChange }: ImportStudentsDialogProps) {
  ...
  const parseCSV = async (text: string) => {
    ...
    // Skip header if present (assuming ID, Name, Email order)
    ...
    for (let i = startIndex; i < lines.length; i++) {
      ...
      // Basic validation for Student ID and Full Name
      if (parts.length >= 2) {
        students.push({
          student_id: parts[0],
          full_name: parts[1],
      ...
      <div className={cn(
        ...
        <div className="p-6 space-y-6 flex-1 overflow-y-auto custom-scrollbar">
          ...
          >
            ...
            {file ? (
              <div>
                <p className="font-semibold text-[#17321A] text-sm truncate">{file.name}</p>
