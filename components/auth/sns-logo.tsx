...
interface SNSLogoProps {
  size?: "sm" | "lg" // Allow flexibility for Dashboard (sm) vs Login (lg)
}
  ...
  return (
    <div className="flex flex-col items-center justify-center text-center">
      <div className={`rounded-full bg-[#00954f]/10 flex items-center justify-center mb-2 ${isLarge ? "p-3 sm:p-4" : "p-2"}`}>
        <GraduationCap className={`text-[#00954f] ${isLarge ? "w-8 h-8 sm:w-10 sm:h-10" : "w-6 h-6"}`} />
      </div>
      
      <h1 className={`font-bold tracking-tight text-[#17321A] font-montserrat ${isLarge ? "text-4xl sm:text-5xl" : "text-2xl"}`}>
        SNS
      </h1>
